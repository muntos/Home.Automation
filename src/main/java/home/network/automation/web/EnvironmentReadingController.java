...
public class EnvironmentReadingController {
    ...
    public String getLivingBalconyTemperatureReadings(@RequestParam(value = "period", required = false) String period){
        return readingService.getReadings(Location.BALCONY_LIVING, Sensor.TEMPERATURE, period);
    }

    @RequestMapping(value = "/balcony/humidity", produces = MediaType.APPLICATION_JSON_VALUE, method = RequestMethod.GET)
    public String getLivingBalconyHumidityReadings(@RequestParam(value = "period", required = false) String period){
        return readingService.getReadings(Location.BALCONY_LIVING, Sensor.HUMIDITY, period);
    }

    @RequestMapping(value = "/outside/temp", produces = MediaType.APPLICATION_JSON_VALUE, method = RequestMethod.GET)
    public String getOutsideTemperatureReadings(@RequestParam(value = "period", required = false) String period){
        return readingService.getReadings(Location.OUTSIDE, Sensor.TEMPERATURE, period);
    }
}