...
public interface EnvironmentReadingRepository extends JpaRepository<EnvironmentReading, Long> {
    ...
    @Query(
            value = "select E.* FROM environment_reading E\n" +
                    "join (\n" +
                    "\tselect DATE_FORMAT(date, '%d-%m-%Y') as day,\n" +
                    "min(value) as value\n" +
                    "    from environment_reading E\n" +
                    "    where E.location = :location and E.sensor = :sensor and E.date >=:date \n" +
                    "    group by DATE_FORMAT(date, '%d-%m-%Y')\n" +
                    "    ) X\n" +
                    "    ON DATE_FORMAT(E.date, '%d-%m-%Y') = X.day\n" +
                    "    AND E.value = X.value order by E.date;",
            nativeQuery = true
    )
    List<EnvironmentReading> findMinValueGroupByDate(@Param("location") String location, @Param("sensor") String sensor,@Param("date") Date date);
    ...
    List<EnvironmentReading> findMaxValueGroupByDate(@Param("location") String location, @Param("sensor") String sensor,@Param("date") Date date);
}
