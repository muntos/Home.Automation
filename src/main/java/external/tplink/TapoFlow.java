...
import external.tplink.domain.Handshake;
import external.tplink.domain.HandshakeParams;
import external.tplink.domain.HandshakeResponse;
import external.tplink.domain.KspKeyPair;
import external.tplink.domain.ParamsBean;
import external.tplink.domain.SecurePassthrough;
import external.tplink.domain.TapoResponse;
import external.tplink.helpers.KspB64;
import external.tplink.helpers.KspDebug;
import external.tplink.helpers.KspHttp;
import external.tplink.helpers.KspJson;
...
public class TapoFlow {
    ...
    public void setPlugState(C658a c658a, String token, String cookie, Boolean deviceOn){
        try{
            DeviceInfoParams deviceInfoParams = new DeviceInfoParams();
            deviceInfoParams.setDeviceOn(deviceOn);
    ...
    public SmartPlug.Status getPlugState(C658a c658a, String token, String cookie) {
        try{
            DeviceInfoParams deviceInfoParams = new DeviceInfoParams();

            TPIoTRequest<DeviceInfoParams> tpIoTRequest = new TPIoTRequest<>();
            tpIoTRequest.setMethod("get_device_info");
            tpIoTRequest.setParams(deviceInfoParams);
            tpIoTRequest.setRequestTimeMils(System.currentTimeMillis());
            tpIoTRequest.setTerminalUUID("88-54-DE-AD-52-E1");
            ...
            TapoResponse tapoResponse = new ObjectMapper().readValue(dec, TapoResponse.class);

            return SmartPlug.value(tapoResponse.getResult().getDeviceOn());
        }
        catch (Exception ex)
        {
            KspDebug.out("Something went wrong! " + ex.getMessage());
            return SmartPlug.Status.UNKNOWN;
        }
    }
    ...
    public void changeStatusLed(C658a c658a, String token, String cookie){
        try{
            ...
            plugDeviceInfoParams.setLedEnable(true);
